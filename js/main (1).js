    ...
    grid.innerHTML = filtered
      ...
      <article class="project-card reveal in-view" data-id="${esc(p.id)}" tabindex="0" role="button" aria-label="Watch ${esc(p.title)}">
        <div class="project-thumb">
          <img src="${esc(p.thumbnail)}" alt="${esc(p.title)} thumbnail" loading="lazy">
...
function getEmbedUrl(type, src) {
  ...
  // Called by the reCAPTCHA API script once it has loaded (see index.html).
  // Explicit render (rather than auto-render) so we control exactly when
  // and where it appears, using the site key from config.js.
  window.initRecaptcha = function () {
    const container = document.getElementById("recaptcha-container");
    const siteKey = (CFG.recaptcha || {}).siteKey || "";
    if (!container || !siteKey || siteKey === "YOUR_RECAPTCHA_SITE_KEY_HERE") {
      return; // no key configured yet — form still works, just without the widget
    }
    recaptchaWidgetId = grecaptcha.render(container, { sitekey: siteKey });
  };
  ...
  function wireContactForm() {
    ...
    form.addEventListener("submit", (e) => {
      ...
      // value, silently drop the submission (no error shown to bots).
      ...
      // reCAPTCHA: only checked if a widget actually rendered (i.e. a site
      // key is configured in config.js). Blocks the vast majority of bots.
      if (recaptchaWidgetId !== null) {
        const recaptchaResponse = grecaptcha.getResponse(recaptchaWidgetId);
        if (!recaptchaResponse) {
          note.textContent = "Please check the \u201cI'm not a robot\u201d box before sending.";
          return;
        }
      }
